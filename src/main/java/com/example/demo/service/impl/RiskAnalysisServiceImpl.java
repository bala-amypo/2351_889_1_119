...
public class RiskAnalysisServiceImpl implements RiskAnalysisService {
    ...
    public RiskAnalysisResult analyzePortfolio(Long portfolioId) {
        ...
        double highestStockPercentage = holdings.stream()
                .mapToDouble(h ->
                        (h.getMarketValue() / totalMarketValue) * 100
                )
                .max()
                .orElse(0.0);

        boolean isHighRisk = highestStockPercentage > 50;

        RiskAnalysisResult result = new RiskAnalysisResult();
        result.setPortfolio(holdings.get(0).getPortfolio());
    ...
    public RiskAnalysisResult getAnalysisById(Long id) {
        throw new UnsupportedOperationException(
                "Persist RiskAnalysisResult to DB using repository");
    }
